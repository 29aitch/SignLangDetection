...
def translated_text(text, target_language= 'en'):
...
def switch_language():
    ...
    if current_language == 'en':
        current_language = 'es'  # Switch to Spanish, you can modify this to other languages
    else:
        current_language = 'en'  # Switch back to English
    print(f"Language switched to {current_language}")

def process_gesture_output(output):
    global current_language
    translated_output = translated_text(output, target_language=current_language)
    print(f"Original: {output}, Translated: {translated_output}")
    return translated_output

def handle_keyboard_input(key):
    if key == ord('L') or key == ord('l'):  # Press 'L' to switch language
        switch_language()
...
def get_output(idx):
    ...
    translated_word = process_gesture_output(word)
...
def get_output(idx):
    ...
    # Ensure this calls the correct translation function
    translated_word = translated_text(word)  # Use translate_text (or translated_text if named differently)
...
if __name__ == '__main__':
    ...
    with mp_hands.Hands(
        ...
        while capture.isOpened():
            ...
            output_size = cv2.getTextSize(output_text, FONT, 0.7, 2)[0]
            cv2.rectangle(image, (5, 0), (10 + output_size[0], 10 + output_size[1]), YELLOW, -1)
            cv2.putText(image, output_text, (10, 15), FONT, 0.7, BLACK, 2)

            mode_text = f"Number: {numberMode}"
            mode_size = cv2.getTextSize(mode_text, FONT, 0.7, 2)[0]
            cv2.rectangle(image, (5, 45), (10 + mode_size[0], 10 + mode_size[1]), YELLOW, -1)
            cv2.putText(image, mode_text, (10, 40), FONT, 0.7, BLACK, 2)
